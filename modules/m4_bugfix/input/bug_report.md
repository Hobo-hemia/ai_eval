...
5. **告警失败后状态污染（P1）**  
   某些请求告警发送失败后，系统仍认为“已告警”，导致后续不再重试，风险告警永久丢失。
...
3. 对同一天，阈值告警必须是 **exactly-once-on-success**：
   - 告警成功后不重复；
   - 告警失败后必须可重试，直到成功。
4. 不能在持锁区执行慢 I/O（包括 notifier 调用）。
5. 告警触发时必须正确处理 `ctx` 取消，避免将失败场景误标为已告警。
6. 若告警触发但 notifier 返回失败（含 `context.Canceled`），本次调用应返回 error，且不应提交总额变更（避免账务与告警状态不一致）。
7. 返回值应为当日累计金额，且负数/零金额按非法输入处理。
8. 保持最小侵入修复，不改变对外函数签名。
9. 修复代码必须在关键修复处带有：
   - `// BUGFIX: [根因与修复逻辑]`
...
- 覆盖以下边界：
  ...
  - 重入 notifier 无死锁
  - overflow 防护
  - 阈值告警 exactly-once-on-success
  - 告警失败可重试
  - `context.Canceled` 后可恢复重试
  - 非法金额（<=0）
